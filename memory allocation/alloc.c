...
void *calloc(size_t num, size_t size) {
    ...
    if (md_plus_request % BLOCK_MULTIPLE != 0) { // ensures all blocks always multiples of BLOCK_MULTIPLE
        ...
        while (h) {
            if (h->total_size >= adj_size) {
                ...
                */
                ...
                //void * user_mem = h_void + sizeof(metadata_t);
    ...
    if (calloc_mem == (void*) -1) { return NULL; }
    ...
    if (calloc_mem == start_of_heap) {    // FIRST request
        md->prev = NULL;
        head_struct = md;
        tail_struct = md;
    }
...
void *malloc(size_t size) {
    ...
    // not sure this matters if I'm not using pointer arithmetic?
    if (md_plus_request % BLOCK_MULTIPLE != 0) { // ensures all blocks always multiples of BLOCK_MULTIPLE
        adj_size = ((md_plus_request / BLOCK_MULTIPLE) + 1) * BLOCK_MULTIPLE;
    }
    
    if (free_blocks > 0) {  // search through LL of free blocks

        metadata_t * h = free_blocks_head;
        
        while (h) {
            if (h->total_size >= adj_size) {  // take first available match
    ...
    if (malloc_mem == (void*) -1) { return NULL; }
    ...
    if (malloc_mem == start_of_heap) {    // FIRST request
        md->prev = NULL;
        head_struct = md;
        tail_struct = md;
    }
...
void free(void *ptr) {
    ...
    if (ptr) {
        ...
        if (previous_block && previous_block->free) { // Left block free?

            if (next_block && next_block->free) { // Right block *also* free?
        ...
        else if (next_block && next_block->free) {   // coalecse ONLY w/ next block
        ...
        else {  // no coalescing possible, manually add into linked list of free blocks
            
            if (free_blocks_head == NULL) { // adding to empty free block list
                free_blocks_head = block_to_free;
                free_blocks_tail = block_to_free;
            }

            else if (block_to_free < free_blocks_head) {    // adding to front of free blocks list
                free_blocks_head->prev_free = block_to_free;
                block_to_free->next_free = free_blocks_head;
                free_blocks_head = block_to_free;
            }

            else if (block_to_free > free_blocks_tail) {    // adding to end of free blocks list
                free_blocks_tail->next_free = block_to_free;
                block_to_free->prev_free = free_blocks_tail;
                free_blocks_tail = block_to_free;
            }

            else {  // placing somewhere in the middle
                //int done = 0;
...
void *realloc(void *ptr, size_t size) {
    ...
    if (new_request_size <= existing->total_size) {
        ...
        // do we bother splitting if it's staying in the same spot?
        // return split_extra(shifted_ptr, existing, new_request_size);
    }

    // if not, will abosrbing immediate right work?
    ...
    /*
    if (adj_size % BLOCK_MULTIPLE != 0) {
        adj_size = ((new_request_size / BLOCK_MULTIPLE) + 1) * BLOCK_MULTIPLE;
    }
    */

    size_t num_chars_to_copy = size;
    if (existing->request_size < size) { num_chars_to_copy = existing->request_size; }
