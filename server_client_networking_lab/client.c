...
void close_server_connection() {
    ...
    if (shutdown(serverSocket, SHUT_RDWR) != 0) { perror(NULL); exit(1); }
    close(serverSocket);
}
...
int connect_to_server(const char *host, const char *port) {
    ...
    if (sock_fd == -1) { perror(NULL); freeaddrinfo(result); exit(1); }

    /*QUESTION 7*/
    int ok = connect(sock_fd, result->ai_addr, result->ai_addrlen);
    if (ok == -1) { perror(NULL); freeaddrinfo(result); exit(1); }
