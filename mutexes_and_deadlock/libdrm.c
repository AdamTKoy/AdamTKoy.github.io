...
typedef struct drm_t { pthread_mutex_t m; } drm_t;
...
int drm_post(drm_t *drm, pthread_t *thread_id) {    // post equivalent to UNlock
    /* Your code here */

    pthread_mutex_lock(&graph_lock);

    if (graph_contains_vertex(sg, (void*)thread_id)) {  // thread already in graph
...
int drm_wait(drm_t *drm, pthread_t *thread_id) {    // wait equivalent to LOCK
    /* Your code here */
    ...
    if (graph_vertex_degree(sg, (void*)drm) > 0) {
        ...
        if (graph_adjacent(sg, (void*)drm, (void*)thread_id)) {
            ...
            //printf("rejecting %p's attempt to double lock\n", thread_id);
            pthread_mutex_unlock(&graph_lock);
            return 0;
        }
    }
    ...
    if (check_cycle(sg, (void*)thread_id, new_set)) {   // cycle detected!
