...
int minixfs_chown(file_system *fs, char *path, uid_t owner, gid_t group) {
    ...
    if (owner != ((uid_t)-1)) { nody->uid = owner; }
    if (group != ((gid_t)-1)) { nody->gid = group; }
...
inode *minixfs_create_inode_for_path(file_system *fs, const char *path) {
	...
	inode * nody = get_inode(fs, path);
	if (nody != NULL) { errno = EEXIST; return NULL; }
    ...
    if (parent_blocks_full < NUM_DIRECT_BLOCKS) {       // add to direct blocks
        ...
        if (parent_blocks_partial_offset != 0) {    // partial available
            db_num = parent->direct[parent_blocks_full];
        }
        
        else {          // no existing/partial available, start new
            db_num = add_data_block_to_inode(fs, parent);
            if (db_num == -1) { errno = ENOSPC; return NULL; }
        }
    }

    else {  // add to indirect blocks

        // first check if we still need to populate the indirect block
		if (parent->indirect == UNASSIGNED_NODE) {
            if (add_single_indirect_block(fs, parent) == -1) { errno = ENOSPC; return NULL; }
        }
...
ssize_t minixfs_virtual_read(file_system *fs, const char *path, void *buf,
    ...
    if (!strcmp(path, "info")) {
        ...
        for (uint64_t i = 0; i < fs->meta->dblock_count; i++) {
            if (GET_DATA_MAP(fs->meta)[i] == 1) { num_used++; }
        }
        ...
        if ((size_t)*off >= strlen(bis)) { return 0; }

        // num_to_copy is minimum between (strlen(bis) - *off) and count
        if (strlen(bis) - *off < count) { num_to_copy = strlen(bis) - *off; }
...
ssize_t minixfs_write(file_system *fs, const char *path, const void *buf,
    ...
    // set error?
	if ((size_t)*off + count > (16 * KILOBYTE * (NUM_DIRECT_BLOCKS + NUM_INDIRECT_BLOCKS))) { errno = ENOSPC; return -1; }

    // calculate how many blocks are needed (including any partial)
	size_t block_count = ((size_t)*off + count) / sizeof(data_block);
	if (((size_t)*off + count) % sizeof(data_block) != 0) block_count++;
	...
	if (mbc == -1) { errno = ENOSPC; return -1; }
    ...
    while (bytes_written < count) {
        if (block_idx < NUM_DIRECT_BLOCKS) {        // write to DIRECT blocks
            current = fs->data_root + (nody->direct[block_idx]);
        }
        else {                                      // write to INDIRECT blocks
        ...
        if (first) {
            ...
            if (write_size > count) { write_size = count; }
    ...
    if (*off + count > nody->size) { nody->size = *off + count; }
...
ssize_t minixfs_read(file_system *fs, const char *path, void *buf, size_t count,
	...
	if ((uint64_t)*off >= nody->size) { return 0; }
    ...
    //printf("starting block index is %zu\n", block_idx);
    //printf("new starting offset is %zu\n", new_offset);

    // TOTAL bytes to read
    size_t bytes_to_read = MIN(count, nody->size - *off);

    //printf("count is %zu\n", count);
    //printf("node size - *offset is %zu\n", (size_t)(nody->size - *off));
    //printf("bytes_to_read is %zu\n", bytes_to_read);
    ...
    while (bytes_read < bytes_to_read) {
        if (block_idx < NUM_DIRECT_BLOCKS) {        // read to DIRECT blocks
            current = fs->data_root + (nody->direct[block_idx]);
        }
        else {                                      // read to INDIRECT blocks
            indir_dbn = indir_blocks + (block_idx - NUM_DIRECT_BLOCKS);
			current = fs->data_root + (*indir_dbn);
        }
    ...
    //printf("count was %zu, bytes_read was %zu\n", count, bytes_read);
