...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "i:o:b:c:p:k:")) != -1) {
		switch(opt) {
            case 'i':
                ...
                if (input_file == NULL) { print_invalid_input(optarg); exit(1); }
                break;

            case 'o':
                // w+ opens or creates file for both reading and writing
                output_file = fopen(optarg, "w+");
                if (output_file == NULL) { print_invalid_output(optarg); exit(1); }
                break;
    ...
    // error check? fseek returns 0 on success, non-zero o.w.
    ...
    if (input_file == stdin) {
        while (feof(input_file) == 0) {     // feof returns non-zero once it tries to read beyond the end of a file

            // check for blocks_to_copy limit
            if (blocks_to_copy != -1 && (full_out + partial_out >= (size_t)blocks_to_copy)) { break; }
            ...
            if (read_bytes < block_size) { partial_in++; }
            else { full_in++; }

            //write_bytes = fread(output_file, 1, read_bytes, input_file);
            write_bytes = fwrite(buf, 1, read_bytes, output_file);

            if (write_bytes < block_size) { partial_out++; }
            else { full_out++; }
