...
//static volatile int caught_ctrl_c = 0;
static pid_t current_child = (pid_t)-1;
//static volatile int interrupt_child = 0;
...
int shell(int argc, char *argv[]) {
    ...
    // Does this *only* need to be here?
    ...
    // is this necessary for parent process?
    ...
    char *buf = (char *)malloc(100*sizeof(char)); // is 100 enough for a directory path?
    ...
    if (is_file) {     // -f
        ...
        if (!file) { print_script_file_error(); exit(1); }

        while (1) {

            bytesread = getline(&line, &capacity, file);
            
            if (bytesread == -1) break;     // EOF breaks loop
            ...
            if (strstr(line, logic_and) || strstr(line, logic_or) || strstr(line, logic_both)) {
                ...
                else { // process external command in child process
                    ext_cmd(line_dup, split_cmd, proc_list);
                }

                free(line_dup);
                if (split_cmd) { vector_destroy(split_cmd); }
            }
        }   // end while loop (-f)
        ...
        fclose(file);
        if (is_hist) { write_history(v_hist, hist_file); }
        ...
        while (1) {
            ...
            // current_child starts off as -1
            // printf("value of current_child: %d\n", current_child);
            // signal(SIGINT, handler); --> relocated toward start of shell
            ...
            if (strlen(line) != 1) {    // if blank line, will automatically prompt again
                ...
                if (strncmp(line, "kill", 4) == 0) {
                    ...
                    if (vector_size(split_cmd) < 2) {      // missing pid
                ...
                // reference: https://stackoverflow.com/questions/34575285/read-proc-stat-information
                    ...
                    for (i = 0; i < vector_size(proc_list); i++) {
                        ...
                        // script file error?
                        if (fp == NULL) { print_script_file_error(); }
                            ...
                            // *this will print w/ parenthesis around command and max of ONE word
                            // printf("cmdx is %s\n", cmdx);
                            // x->command = cmdx;
                ...
                // check for LOGICAL
                ...
                else if (strstr(line, "&")) {       // background process (always external cmd)
                    vector_push_back(v_hist, line);
                    ...
                    if (child == 0) {
                        ...
                        else if(WIFEXITED(status) == 0) {    // only print if *not* SIGINT?
                            failure = 1;
                            print_wait_failed();
                        }
                        */
                ...
                // REDIRECTION - INPUT "<"
                    ...
                    if (fd_in == -1) { print_redirection_file_error(); }
                    ...
                    if (child == 0) {
                        ...
                        else if (WIFEXITED(status) == 0) {  // parent failed to wait
                            print_wait_failed();
                            for (size_t i = 0; i < vector_size(proc_list); i++) {
                                process_info * pi = vector_get(proc_list,i);
                                if (pi->pid == child) {
                ...
                else {  // No logical or redirection operators
...
void change_directory(char * path, char * buffer) {
    ...
    int cdir = chdir(path);
    if (cdir != 0) { print_no_directory(path); }
    else { getcwd(buffer,100); }
...
void process_logical(char * lineptr, char * buffer, vector * process_list) {
    ...
    if (strstr(lineptr, l_and)) {   // found 'AND'
        logic_split = string_split(lineptr, l_and);
        chose_and = 1;
    }
    else if (strstr(lineptr, l_or)) {  // found 'OR'
    ...
    for (idx = 0; idx < 2; idx++) {
        ...
        if (strncmp(cmd, "cd", 2) == 0) {
            ...
            int pgs = setpgid(child, child);  // each child gets its own group?
            if (pgs == -1) { print_setpgid_failed(); }
            ...
            if (child == 0) {
                ...
                if (WIFSIGNALED(status)) {  // child killed via SIGINT
                    for (size_t i = 0; i < vector_size(process_list); i++) {
                        process_info * pi = vector_get(process_list,i);
                        if (pi->pid == child) {
                            vector_erase(process_list, i);
                    ...
                    if (chose_and && idx == 0) {
                        //printf("chose_and: 1st failed (wait), skipping 2nd\n");
                        current_child = -1;
                        break;
                    }
                }
                    ...
                    if (chose_and && idx == 0) {
                        //printf("chose_and: 1st failed, skipping 2nd\n");
                        current_child = -1;
                        break;
                    } 
                }

                else if (WIFEXITED(status)) {  // successful child termination
                    for (size_t i = 0; i < vector_size(process_list); i++) {
                        process_info * pi = vector_get(process_list,i);
                        if (pi->pid == child) {
                            vector_erase(process_list, i);
...
void process_redirection_output(vector * output_cmd, char * redir_file, vector * process_list) {
    ...
    if (child == 0) {
        ...
        int status = 0;
        waitpid(child, &status, 0);
        if (WIFSIGNALED(status)) {  // if child killed via SIGINT
            for (size_t i = 0; i < vector_size(process_list); i++) {
                process_info * pi = vector_get(process_list,i);
                if (pi->pid == child) {
                    vector_erase(process_list, i);
...
// ref: https://www.decompile.com/cpp/faq/fopen_write_append.htm
void write_history(vector * v, char * hist_filename) {
