...
// run through valid paths of tree for each search word
// should I make 'input' const?
void searchTree(size_t offset, const char * word) {

  int status = fseek(in_file, offset, SEEK_SET);
  if (status != 0) { formatFail(filename); exit(2); }
  ...
  // get 'word' at nody (is 256 too much? I mean, what word is that long...)
  char nody_word[256];
  ...
  else if (compare_value < 0) {   // word is < this node's word: check left child
    
    // no left child exists, end
    if (nody.left_child == 0) { printNotFound(word); }

    // or recurse on existing left child
    else { searchTree(nody.left_child, word); }
  }

  else {  // word is > this node's word: check right child

    // no right child exists, end
    if (nody.right_child == 0) { printNotFound(word); }

    // or recurse on existing right child
    else { searchTree(nody.right_child, word); }
  }
}
...
int main(int argc, char **argv) {
    ...
    if (!in_file) { openFail(argv[1]); exit(2); }
    ...
    // 'open' fail vs. 'format' fail?
    if (first_four != 1) { formatFail(filename); exit(2); }
    if (strncmp(first_four_buf, BINTREE_HEADER_STRING, BINTREE_ROOT_NODE_OFFSET) != 0) { formatFail(filename); exit(2); }

    // (argc - 2) should == number of search words
