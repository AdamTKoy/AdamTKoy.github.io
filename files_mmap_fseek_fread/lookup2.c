...
void searchTree(size_t offset, const char * word) {
  ...
  if (compare_value == 0) { printFound(word, nody->count, nody->price); }
...
int main(int argc, char **argv) {
  ...
  // create mapping
  // example usage from course book: 
  ...
  if (strncmp(addr, BINTREE_HEADER_STRING, BINTREE_ROOT_NODE_OFFSET) != 0) { formatFail(filename); exit(2); }
